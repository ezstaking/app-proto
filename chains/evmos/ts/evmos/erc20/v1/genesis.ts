...
export const Params = {
  ...
  fromJSON(object: any): Params {
    ...
    if (isSet(object.enableErc20))
      obj.enableErc20 = Boolean(object.enableErc20);
    if (Array.isArray(object?.nativePrecompiles))
      obj.nativePrecompiles = object.nativePrecompiles.map((e: any) =>
        String(e),
      );
    if (Array.isArray(object?.dynamicPrecompiles))
      obj.dynamicPrecompiles = object.dynamicPrecompiles.map((e: any) =>
        String(e),
      );
    return obj;
  },
  toJSON(message: Params): JsonSafe<Params> {
    ...
    if (message.nativePrecompiles) {
      obj.nativePrecompiles = message.nativePrecompiles.map((e) => e);
    } else {
      obj.nativePrecompiles = [];
    }
    if (message.dynamicPrecompiles) {
      obj.dynamicPrecompiles = message.dynamicPrecompiles.map((e) => e);
    } else {
      obj.dynamicPrecompiles = [];
    }
    return obj;
  },
