...
export const TableDescriptor = {
  ...
  fromJSON(object: any): TableDescriptor {
    ...
    if (isSet(object.primaryKey))
      obj.primaryKey = PrimaryKeyDescriptor.fromJSON(object.primaryKey);
    if (Array.isArray(object?.index))
      obj.index = object.index.map((e: any) =>
...
export const SecondaryIndexDescriptor = {
  ...
  toJSON(
    message: SecondaryIndexDescriptor,
  ): JsonSafe<SecondaryIndexDescriptor> {
    const obj: any = {};
    message.fields !== undefined && (obj.fields = message.fields);
    message.id !== undefined && (obj.id = Math.round(message.id));
